 ...
 */
const TYPE_ORDER: EquipmentItemType[] = [
  'SKI',
  'SKI_BOOT',
  'SNOWBOARD',
  'SNOWBOARD_BOOT',
...
export const PickUpPersonCard = ({ person }: PickUpPersonCardProps) => {
  ...
  const items = person.reservationItems
    ...
    .sort(
      (a, b) =>
        TYPE_ORDER.indexOf(a.equipmentItem.type) -
        TYPE_ORDER.indexOf(b.equipmentItem.type)
    )
  ...
  return (
    <div className="bg-background rounded-lg border p-3">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        {person.level && (
          <Badge variant="outline">{LEVEL_LABELS[person.level]}</Badge>
        )}
      </div>

      {items.length === 0 ? (
        <p className="text-muted-foreground text-sm">
          Této osobě není přiřazeno žádné vybavení.
        </p>
      ) : (

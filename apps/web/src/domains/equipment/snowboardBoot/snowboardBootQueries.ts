	...
	useMutation(
		trpc.equipment.snowboardBoot.create.mutationOptions({
			onSuccess: () => {
				...
				notifySuccess('Snowboardová bota přidána', 'Snowboardová bota byla úspěšně vytvořena.')
			},
			onError: (error) => notifyError(error.message, 'Nepodařilo se vytvořit snowboardovou botu.'),
		})
	)
...
export const useUpdateSnowboardBoot = () =>
	useMutation(
		trpc.equipment.snowboardBoot.update.mutationOptions({
			onSuccess: () => {
				invalidateSnowboardBootList()
				notifySuccess('Snowboardová bota upravena', 'Snowboardová bota byla úspěšně upravena.')
			},
			onError: (error) => notifyError(error.message, 'Nepodařilo se upravit snowboardovou botu.'),
		})
	)

...
export const useCreateSkiBoot = () =>
	useMutation(
		trpc.equipment.skiBoot.create.mutationOptions({
			...
			onError: (error) => notifyError(error.message, 'Nepodařilo se vytvořit lyžařskou botu.'),
		})
	)
...
export const useUpdateSkiBoot = () =>
	useMutation(
		trpc.equipment.skiBoot.update.mutationOptions({
			...
			onError: (error) => notifyError(error.message, 'Nepodařilo se upravit lyžařskou botu.'),
		})
	)

...
export const equipmentItemRouter = router({
	retire: publicProcedure.input(equipmentIdInputSchema).mutation(async ({ input }) => {
		return await retireEquipmentItem(input)
	}),
	unretire: publicProcedure.input(equipmentIdInputSchema).mutation(async ({ input }) => {
		return await unretireEquipmentItem(input)
	}),
	delete: publicProcedure.input(equipmentIdInputSchema).mutation(async ({ input }) => {
		return await deleteEquipmentItem(input)
	}),
	findAvailable: publicProcedure.input(findAvailableInputSchema).query(async ({ input }) => {
